...
//    char basedir[MAX_FILENAME];
    ...
    SST* sst;
    MemTable* memtable;
   // pthread_mutex_t lock; // NOTE: declaration of DB mutex lock  // used for part 2 only
    pthread_mutex_t wrl; // NOTE: declaration of mutex lock used for readers/writers
    pthread_cond_t can_read; // NOTE: declaration of conditional variable used for readers
    pthread_cond_t can_write; // NOTE: declaration of conditional variable used for writers
    int readers; // NOTE : count of readers in critical area
    int writers; // NOTE: count of writers in critical area (0-1)
} DB;
