...
static void merge_thread(void* data)
{
    ...
    while (1)
    {
        ...
        sst->merge_state = 0;
	INFO("Merge thread finished its job");
        pthread_cond_signal(&sst->cv); // NOTE: Merge thread signals the main thread in sst_ merge to continue working
        pthread_mutex_unlock(&sst->cv_lock);
    }
}
#endif
...
#ifdef BACKGROUND_MERGE
{
    ...
    pthread_mutex_unlock(&self->cv_lock);
    pthread_mutex_lock(&self->cv_lock);
    //pthread_mutex_lock(&self->mr_lock); // NOTE: locking mutex for cond_wait while we wait for merge_thread to finish its job and signal back // used for part 2, 
    																//switched to original cv_lock
    //INFO("Thread %ld waiting for merge thread to finish", pthread_self());
    while(self->merge_state!=0) // NOTE: waiting for merge_state to change to 0 by the merge_thread thread
    {
	pthread_cond_wait(&self->cv, &self->cv_lock); // NOTE: waiting for merge_thread thread to signal
    }    
    pthread_mutex_unlock(&self->cv_lock);
}
