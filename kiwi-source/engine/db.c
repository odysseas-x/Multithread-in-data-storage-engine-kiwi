...
DB* db_open_ex(const char* basedir, uint64_t cache_size)
{
   ...
   // pthread_mutex_init(&self->lock, NULL); // NOTE: initializing mutex // used for part 2 only
    pthread_mutex_init(&self->wrl, NULL); // NOTE: initializing mutexes and conditional variables
    pthread_cond_init(&self->can_read, NULL);
    pthread_cond_init(&self->can_write, NULL);
    self->readers=0; // NOTE: initializing readers, writers at 0
    self->writers=0;
...
{	// NOTE: we have created a readers priority queue, many readers-1 writer at a time
   // pthread_mutex_lock(&self->lock); // NOTE: locking mutex for write // used for part 2 only
    pthread_mutex_lock(&self->wrl); // NOTE: locking area for individual thread check
    while(self->readers > 0 || self->writers > 0) // NOTE: checking for readers/writers in queue or critical area
	    pthread_cond_wait(&self->can_write, &self->wrl);
    self->writers = 1; // NOTE: 1 writer pass to critical area (ONLY) 
    pthread_mutex_unlock(&self->wrl);
    ...
    int tmp; // NOTE: keeping the return value of memtable_add 
    INFO("Thread %ld adding key", pthread_self()); // NOTE: reporting a thread adding a key
    tmp=memtable_add(self->memtable, key, value); // NOTE: adding the key while still in mutex 
   // pthread_mutex_unlock(&self->lock); // NOTE: unlocking mutex // used for part 2 only
    pthread_mutex_lock(&self->wrl);
    self->writers = 0; // NOTE: reporting writer has finished
    pthread_cond_broadcast(&self->can_read); // NOTE: letting readers start if any
    pthread_cond_signal(&self->can_write); // NOTE: letting the next writer start if any
    pthread_mutex_unlock(&self->wrl);
    return tmp;
}
...
{	// NOTE: we have created a readers priority queue, many readers-1 writer at a time
   // pthread_mutex_lock(&self->lock); // NOTE: locking mutex for read // used for part 2 only
    pthread_mutex_lock(&self->wrl);
    while(self->writers) // NOTE: checking for writer in critical area, if not continue
	    pthread_cond_wait(&self->can_read, &self->wrl);
    self->readers++; // NOTE: keeping track of readers to signal writers when done (if any)
    pthread_mutex_unlock(&self->wrl);
    INFO("Thread %ld searching key", pthread_self()); // NOTE: reporting a thread searching a key
    if (memtable_get(self->memtable->list, key, value) == 1){
	//pthread_mutex_unlock(&self->lock); // NOTE: unlocking mutex if we find key in memtable // used for part 2 only
	pthread_mutex_lock(&self->wrl); 
	self->readers--; // NOTE: reader has finished
	if(!self->readers)
		pthread_cond_signal(&self->can_write); // NOTE: letting writers in queue start (if any)
	pthread_mutex_unlock(&self->wrl);
        return 1;
    }
    int tmp;
    tmp=sst_get(self->sst, key, value); // NOTE: getting the key from SST to return
    // pthread_mutex_unlock(&self->lock); // NOTE: unlocking mutex // used for part 2 only
    pthread_mutex_lock(&self->wrl); 
    self->readers--; // NOTE: same logic as if key found in memtable
    if(!self->readers) 
	    pthread_cond_signal(&self->can_write);
    pthread_mutex_unlock(&self->wrl);
    return tmp;
}
